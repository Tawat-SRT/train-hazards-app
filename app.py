...
# 1. ตั้งค่าหน้าจอ
st.set_page_config(page_title="Train Hazards Dashboard V1.10", page_icon="🚆", layout="wide")
...
st.markdown(f"""
    ...
    .stDataFrame, .stPlotlyChart {{ background-color: rgba(255,255,255,0.85); border-radius: 10px; padding: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }}
    
    /* 🌟 เพิ่มส่วนตกแต่งสำหรับ Footer (เครดิตผู้ออกแบบ) */
    .app-footer {{
        text-align: center;
        padding: 20px;
        margin-top: 40px;
...
st.markdown('<p class="dashboard-subtitle">ระบบรายงานและเฝ้าระวังอุบัติเหตุรถไฟเฉี่ยวชนสัตว์แบบเรียลไทม์</p>', unsafe_allow_html=True)
...
st.markdown("""
    <div class="app-footer">
        ...
        <span style="color: gray; font-size: 12px;">Version 1.10</span>
    </div>
""", unsafe_allow_html=True)
