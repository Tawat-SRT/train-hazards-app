...
# 1. ตั้งค่าหน้าจอ
st.set_page_config(page_title="Train Hazards Dashboard V1.11", page_icon="🚆", layout="wide")
...
st.info("💡 **วิธีใช้งาน:** คุณสามารถคลิกที่เซลล์ในตารางด้านล่างเพื่อพิมพ์แก้ไขข้อมูล หรือติ๊กเลือกกล่องหน้าสุดของแถวเพื่อลบ จากนั้นกดปุ่ม 'อัปเดตและบันทึกการแก้ไข' เพื่อให้ระบบคำนวณใหม่")

# 💡 ใช้ st.data_editor แทน st.dataframe เพื่อเปิดสิทธิ์การแก้ไข
edited_df = st.data_editor(
    st.session_state.hazard_data,
    use_container_width=True,
    num_rows="dynamic", # อนุญาตให้เพิ่มแถว หรือลบแถวได้
    height=250,
    key="editor"
)

# ปุ่มสำหรับบันทึกการแก้ไขที่ทำในตาราง
if st.button("🔄 อัปเดตและบันทึกการแก้ไข (Update Changes)", use_container_width=True):
    st.session_state.hazard_data = edited_df
    st.success("✅ บันทึกการแก้ไขข้อมูลเรียบร้อยแล้ว! แผนที่และสถิติอัปเดตแล้ว")
    st.rerun()

# ==========================================
# ส่วนที่ 4: ฟอร์มเพิ่มข้อมูลใหม่
# ==========================================
...
st.markdown("""
    <div class="app-footer">
        ...
        <span style="color: gray; font-size: 12px;">Version 1.11</span>
    </div>
""", unsafe_allow_html=True)
