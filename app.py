...
with c2:
    ...
    lat_center = df['Latitude'].mean() if not df['Latitude'].isna().all() else 13.7367
    lon_center = df['Longitude'].mean() if not df['Longitude'].isna().all() else 100.5231
...
# ตารางจัดการข้อมูลแบบ Real-time
st.markdown('<p class="section-header">✏️ จัดการและเพิ่มข้อมูล</p>', unsafe_allow_html=True)
edited_df = st.data_editor(st.session_state.hazard_data, use_container_width=True, num_rows="dynamic", key="data_editor")

# ปุ่มบันทึกและสั่ง rerun ทันที
if st.button("🔄 บันทึกการแก้ไข (Update Real-time)", use_container_width=True):
